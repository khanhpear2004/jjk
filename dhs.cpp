...
class imp_res : public Restaurant
{
	public:
		...
		bool onService(customer *c){
			...
			tmp = nullptr; l = nullptr; delete tmp; delete l;
		...
		void swap(customer *&a, customer *&b){
			if(a->next == b){
				...
				aNext = nullptr; aPrev = nullptr; bNext = nullptr; bPrev = nullptr;
				delete aNext; delete aPrev; delete bNext; delete bPrev;
			}
		}
...
void imp_res :: printSubSmallest(customer *r){
	...
	for(int i = 0; i < length; i++){
		int min = smallest->energy; customer *k = smallest;
		if(min >= k->energy){
			min = k->energy; smallest = k;
		}
		k = k->next;
	}
